...
class WorkshopState(BaseState):
    ...
    def init_ui(self):
        ...
        # 3. Physics Sliders (Left Column - X=50)
        ...
        # 5. Global Sliders (Right Column 1 - X=850)
        x_right_a = 880
        ...
        # 6. Directions (Right Column 2 - Diamond Pad)
        dx, dy = 1350, 290
        ...
        # 7. Metadata Inputs (Right Column 2 - Below Directions)
        my = dy + 180 
    ...
    def startup(self, persistent):
        # 1. Load Data directly from passed context
        self.db_source_config = persistent.get("config", {}).copy()
        
        # --- NEW: Use passed Name/Origin ---
        ...
        # 3. Create Baseline (To check for dirty state later)
    ...
    def add_row_to_ui(self, t, s, tol):
        ...
        self.timeline_rows.append({
            "time": TextInput(sx, y, 60, 30, str(t)),
            "speed": TextInput(sx + 70, y, 90, 30, str(s)),
            "tol": TextInput(sx + 170, y, 60, 30, str(tol)),
            "del": IconButton(sx + 240, y, 30, 30, "X")
        })

    def get_current_data(self):
        data = self.db_source_config.copy()

        # Bake in current sliders
        data.update({
            ...
            # Bake in current name (important for exports)
    ...
    def handle_event(self, event):
        ...
        if self.modal:
            ...
            if res == "OK":
                ...
                # Saving to Local -> It is now LOCAL origin
        ...
        # --- EXIT ON ESCAPE ---
        # If no modal is open, ESC goes back to Hub
        ...
        if self.btn_save.handle_event(event):
            # Only allow overwrite if it was ALREADY Local
            if self.original_origin == "LOCAL":
                new_data = self.get_current_data()
                new_data["name"] = self.original_name # Keep existing name on overwrite
                
                # Delete old hash version, save new version
    ...
    def draw(self, screen):
        ...
        # Identity Logic
        icon = "📄"
        if self.original_origin == "OFFICIAL": icon = "🌐"
        elif self.original_origin == "COMMUNITY": icon = "👥"
        elif self.original_origin == "LOCAL": icon = "🛠️"
        
        display_name = self.original_name
        if dirty: 
            display_name += " *" # Subtle dirty indicator
            title_color = (255, 140, 0)
        else:
            title_color = UI_COLOR
        ...
        for s in [self.sl_smooth, self.sl_zoom, self.sl_warmup_time, self.sl_dur]: 
            s.draw(screen, self.font)
        for t in self.toggles: 
            t.draw(screen, self.font)

        # Draw Left Panel (Physics vs Timeline)
        if self.edit_mode == "SIMPLE":
            self.draw_txt(screen, "PHYSICS (SIMPLE)", hy, UI_COLOR, x=175, font=self.font_big)
            for s in [self.sl_start, self.sl_end, self.sl_tol]: 
                s.draw(screen, self.font)
        else:
